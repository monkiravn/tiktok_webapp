      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-lg">Chờ phê duyệt</h3>
          ...
          <div className="space-y-3">
            {pendingUsers.length === 0 ? (
              <div className="text-center py-8 text-neutral-400">
                ...
                Không có người dùng chờ phê duyệt
              </div>
            ) : (
              pendingUsers.map((u) => (
                <div key={u.id} className="flex items-center justify-between p-4 rounded-lg bg-neutral-800 border border-neutral-700">
                  ...
                  <div className="flex space-x-2">
                    <Button variant="primary" size="sm" onClick={() => approve(u.id)}>Phê duyệt</Button>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-lg">Tất cả người dùng</h3>
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {allUsers.length === 0 ? (
              ...
              allUsers.map((u) => (
                <div key={u.id} className="flex items-center justify-between p-4 rounded-lg bg-neutral-800 border border-neutral-700">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${u.role === 'admin' ? 'bg-purple-600' : u.status === 'approved' ? 'bg-green-600' : 'bg-gray-600'}`}>
                      <span className="text-white font-semibold">{u.username.charAt(0).toUpperCase()}</span>
                    </div>
                    <div>
                      <div className="font-medium">@{u.username}</div>
                      <div className="flex items-center space-x-2 text-sm">
                        <Badge variant={u.role === 'admin' ? 'purple' : 'blue'}>{u.role === 'admin' ? 'Admin' : 'User'}</Badge>
                        <Badge variant={u.status === 'approved' ? 'green' : 'yellow'}>{u.status === 'approved' ? 'Đã duyệt' : 'Chờ duyệt'}</Badge>
                      </div>
                    </div>
                  </div>
                  <Button variant="danger" size="sm" onClick={() => removeUser(u.id)}>Xóa</Button>
